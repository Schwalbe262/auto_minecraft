...
/** One warehouse survey per haul, then reserve deposits or bounded carried-surplus shipment. */
final class TomatoOverflowStorageModule implements AutomationModule {
    ...
    @Override public WorkResult tick(Context c) {
        ...
        switch(stage) {
            case START -> {
                ...
                started=world.tick();day=Math.floorDiv(world.dayTime(),24000L);index=0;stage=Stage.SURVEY;
                if(world.menu().container())close(c,Stage.SURVEY);
            }
            case SURVEY,DEPOSIT,SHIPPING -> {
                List<Poi> targets=stage==Stage.SHIPPING ? shipping : stores;
                if(index>=targets.size()) {
                    if(stage==Stage.SURVEY){index=0;stage=Stage.DEPOSIT;break;}
                ...
                if(stage==Stage.DEPOSIT && readyToShip()) {
                    ...
                    session.tomatoSalePermit=new TomatoSalePermit(held(c),started,day,percent,total(),capacity(),positions());
                    index=0;stage=Stage.SHIPPING;break;
                }
                if(stage==Stage.DEPOSIT && held(c)==0)return finish(c,WorkResult.idle());
                if(stage==Stage.SHIPPING && !saleValid(c))return finish(c,WorkResult.blocked("토마토 잉여 출하 확인이 만료되어 저장량을 다시 확인합니다."));
                ...
                if(nav==Navigation.Result.ARRIVED)submit(c,new Action.UseBlock(targets.get(index).pos(),Action.Use.OPEN_CONTAINER),Pending.OPEN);
            }
            case INSPECT,STORE -> {
                if(!validMenu(c))return finish(c,WorkResult.blocked("토마토 저장 메뉴가 변경되었습니다."));
                List<ItemData> contents=readStorage(c);
                if(contents==null)return finish(c,WorkResult.blocked("잉여 판매 전 토마토 전용 일반 통 27칸을 확인해야 합니다. 다른 내용물은 건드리지 않았습니다."));
            ...
            case SELL -> {
                ...
                if(!TomatoSaleRules.permitted(source.item(),c))return finish(c,WorkResult.blocked("확인한 잉여 토마토 출하량을 마쳤습니다. 새 수량은 다시 확인합니다."));
    ...
    private boolean saleValid(Context c){ItemSlot item=ModuleSupport.inventoryItem(c,i->i.is(ItemData.TOMATO));return item!=null && TomatoSaleRules.permitted(item.item(),c);}
    ...
    @Override public void reset(){if(session!=null)session.tomatoSalePermit=null;stage=Stage.START;pending=null;ticket=-1;menuId=-1;

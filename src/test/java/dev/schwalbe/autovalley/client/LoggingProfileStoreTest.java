...
class LoggingProfileStoreTest {
    ...
    @Test void newProfileKeepsLoggingOffAndAllExistingDefaultsUnchanged() {
        ...
        assertEquals(3, p.schemaVersion);
    ...
    @Test void legacySchemaTwoUpgradesOnlyOnExplicitSaveAndGuardsLoggingProgressFromOldClients() throws Exception {
        ...
        Profile p = store.load(key); assertEquals(3, p.schemaVersion); assertEquals(old, Files.readString(file));
        ...
        Profile loaded = store.load(key); assertEquals(3, loaded.schemaVersion); assertTrue(loaded.loggingRunActive);
